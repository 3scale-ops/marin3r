...
func TestGenerateTLSCertificateSdsConfig(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:    args{},
			want:    `{"resources":[{"@type":"type.googleapis.com/envoy.extensions.transport_sockets.tls.v3.Secret","tlsCertificate":{"certificateChain":{"filename":"/etc/envoy/tls/client/tls.crt"},"privateKey":{"filename":"/etc/envoy/tls/client/tls.key"}}}]}`,
			wantErr: false,
		},
	}
	for _, tt := range tests {
